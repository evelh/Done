...
function SaveNew() {
    ...
    requestData("/Tools/ResourceAjax.ashx?act=AddDepartment&resId=" + $("input[name='con2675']").val() + "&dpt=" + $("#selDpt").val() + "&role=" + $("#selRole").val() + "&dft=" + dft + "&dpt=" + dpt + "&act=" + act, null, function (data) {
        if (data == true) {
            window.location.reload();
        } else {
            LayerMsg("添加失败");
