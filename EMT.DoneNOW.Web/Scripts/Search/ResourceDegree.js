...
$("#addData").hide();
requestData("/Tools/SysSettingAjax.ashx?act=GetSkillCates&type=3", null, function (data) {
    if (data != null) {
        var opt = "";
...
function SaveNew() {
    ...
    requestData("/Tools/ResourceAjax.ashx?act=AddCert&resId=" + $("input[name='con2679']").val() + "&cate=" + $("#selCate").val() + "&complete=" + cplt + "&desc=" + $("#desc").val(), null, function (data) {
        if (data == true) {
            window.location.reload();
        }
    })
