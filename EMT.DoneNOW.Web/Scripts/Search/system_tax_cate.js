...
function Edit() {
    window.open("../SysSetting/TaxCateManage.aspx?id=" + entityid, windowObj.general + windowType.edit, 'left=0,top=0,location=no,status=no,width=700,height=500', false);
}
function Delete() {
    $.ajax({
        type: "GET",
        url: "../Tools/GeneralViewAjax.ashx?act=delete_validate&GT_id=44&id=" + entityid,
        async: false,
        //dataType: "json",
...
function RightClickFunc() {
    $.ajax({
        type: "GET",
        async: false,
        url: "../Tools/GeneralAjax.ashx?act=TaxCateDeleteCheck&id=" + entityid,
        dataType: 'json',
        success: function (data) {
            if (data) {
                $("#DeleteLi").show();
