...
namespace EMT.DoneNOW.DAL
{
    public class d_cost_code_dal : BaseDAL<d_cost_code>
    {
        ...
        {
            return FindListBySql<d_cost_code>($"SELECT * from d_cost_code where id in (SELECT cost_code_id FROM ctt_contract_exclusion_cost_code WHERE contract_id={contract_id} AND delete_time=0) and delete_time = 0 and cate_id = {(int)DTO.DicEnum.COST_CODE_CATE.GENERAL_ALLOCATION_CODE}");
        }
        ...
        {
            return FindListBySql<d_cost_code>($"SELECT * from d_cost_code where id not in (SELECT cost_code_id FROM ctt_contract_exclusion_cost_code WHERE contract_id={contract_id} AND delete_time=0) and delete_time = 0 and cate_id = {(int)DTO.DicEnum.COST_CODE_CATE.GENERAL_ALLOCATION_CODE}");
        }
     }
}
